...
def fetch_chapter_content(chapter_slug: str) -> Optional[dict]:
    ...
    """
    from .openstax_chapters import OPENSTAX_CHAPTERS, CHAPTER_TO_MODULES, get_openstax_url_for_chapter
    ...
    if len(module_ids) > 1:
        with ThreadPoolExecutor(max_workers=min(len(module_ids), 5)) as executor:
            futures = {executor.submit(fetch_module_content_cached, mid): mid for mid in module_ids}
    ...
    if not content_parts:
        ...
        return None
    return {'chapter_slug': chapter_slug, 'title': title, 'url': get_openstax_url_for_chapter(chapter_slug), 'module_ids': module_ids, 'content': '\n\n---\n\n'.join(content_parts)}
...
def fetch_multiple_chapters(chapter_slugs: list[str]) -> list[dict]:
    ...
    with ThreadPoolExecutor(max_workers=min(len(chapter_slugs), 3)) as executor:
        futures = {executor.submit(fetch_chapter_content, slug): slug for slug in chapter_slugs}
        for future in futures:
            try:
                result = future.result()
                if result:
...
async def fetch_modules_for_topic(topic: str, max_modules: int=3) -> dict:
            ...
            if chapter_slugs[0] in CHAPTER_TO_MODULES:
                ...
                for mid in module_ids:
                    if mid in MODULE_INDEX:
                        ...
                        matched_modules.append({'id': mid, 'title': info['title'], 'unit': info['unit'], 'chapter': info['chapter'], 'url': get_module_url(mid)})
            else:
    ...
    if not matched_modules:
        ...
        return {'topic': topic, 'matched_modules': [], 'combined_content': '', 'sources': []}
    ...
    if not contents:
        ...
        return {'topic': topic, 'matched_modules': matched_modules, 'combined_content': '', 'sources': []}
    combined_parts = []
    ...
    return {'topic': topic, 'matched_modules': matched_modules, 'combined_content': '\n\n===\n\n'.join(combined_parts), 'sources': [source_citation]}
...
async def _llm_match_topic_to_chapters(topic: str, max_chapters: int=3) -> list[str]:
    ...
    try:
        ...
        model = os.getenv('GENAI_MODEL', 'gemini-3-flash')
