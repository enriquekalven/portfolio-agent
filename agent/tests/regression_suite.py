...
TEST_CASES = [{'format': 'flashcards', 'context': 'tell me about your AI skills'}, {'format': 'flashcards', 'context': 'how does your experience fit a senior AI lead role?'}, {'format': 'timeline', 'context': 'career progression'}, {'format': 'blog_cards', 'context': 'technical writing'}, {'format': 'video_cards', 'context': 'keynotes and talks'}, {'format': 'awards', 'context': 'recognitions'}, {'format': 'certs', 'context': 'cloud certifications'}, {'format': 'testimonials', 'context': 'what googlers say'}, {'format': 'speaker', 'context': 'speaking events'}, {'format': 'gallery', 'context': 'visual achievements'}]
...
async def run_tests(save_baseline=False):
    ...
    for test_id, current_result in current_outputs.items():
        ...
        if current_result != baseline_result:
            ...
            diff = difflib.unified_diff(baseline_str.splitlines(), current_str.splitlines(), fromfile='baseline', tofile='current', lineterm='')
            print('\n'.join(list(diff)))
