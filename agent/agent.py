...
class LearningMaterialAgent:
    ...
    SUPPORTED_FORMATS = ['flashcards', 'quiz', 'podcast', 'video', 'image', 'timeline', 'video_cards', 'blog_cards', 'awards', 'certs', 'speaker', 'testimonials', 'gallery', 'creative', 'comics']
    ...
    async def generate_content(self, format_type: str, context_topic: str='') -> dict[str, Any]:
        ...
        is_json_format = format_type in ['flashcards', 'quiz', 'image', 'video', 'timeline', 'video_cards', 'blog_cards', 'awards', 'certs', 'speaker', 'testimonials', 'gallery', 'creative']
        cache_name = self._get_cache_name(system_prompt)
        config_args = {'response_mime_type': 'application/json' if is_json_format else 'text/plain'}
        if cache_name:
        ...
        response = client.models.generate_content(model=self.model_id, contents=[types.Content(role='user', parts=[types.Part.from_text(text=user_message)])], config=types.GenerateContentConfig(**config_args))
    ...
    def _get_cache_name(self, system_instruction: str) -> Optional[str]:
        ...
        try:
            ...
            cached_content = client.caches.create(model=model_name, config=types.CachedContentConfig(system_instruction=system_instruction, ttl='3600s'))
        ...
        try:
            ...
            return {'format': format_type, 'a2ui': a2ui_json, 'surfaceId': SURFACE_ID, 'source': source}
        except Exception as e:
    ...
    async def stream(self, message: str, session_id: str='default') -> AsyncGenerator[dict[str, Any], None]:
        ...
        if format_type in self.SUPPORTED_FORMATS:
            ...
            response = client.models.generate_content(model=self.model_id, contents=[types.Content(role='user', parts=[types.Part.from_text(text=message)])], config=types.GenerateContentConfig(**config_args))
